...
class FunctionParser
{
	...
	public function parse(Input $input, &$function, &$classPaths, &$functionPaths)
	{
		$this->input = $input;
		...
		if (
			...
			$this->getEnd($iSignatureEnd, $isDefinition, $classPaths, $functionPaths)
		) {
	...
	private function getEnd(&$iSignatureEnd, &$isDefinition, array &$classPaths, array &$functionPaths)
	{
		...
		while ($this->input->read($type)) {
			...
			if ($type === Lexer::BRACE_LEFT_) {
				...
				if ($this->getBody($classPaths, $functionPaths)) {
					$isDefinition = true;
					return true;
				}
	...
	private function getBody(&$classPaths, &$functionPaths)
	{
		$position = $this->input->getPosition();
		$depth = 1;
