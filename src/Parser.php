...
class Parser
{
	...
	public function parse($input)
	{
		...
		if (!$this->getSuite($fixture, $tests)) {
			return null;
		}
	...
	private function getTests(&$tests)
	{
		...
		while ($this->getTest($subject, $cases)) {
			// TODO: use the line number of the subject comment as the key here:
			$tests[] = array(
				'subject' => $subject,
				'cases' => $cases
			);
	...
	private function getCases(&$cases)
	{
		...
		while ($this->getCase($input, $output)) {
			// TODO: use the line number of the input (or output) comment as the key here:
			$cases[] = array(
				'input' => $input,
				'output' => $output
			);
